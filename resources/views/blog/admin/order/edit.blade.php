...
@section('content')
    ...
    <section class="content-header">
        ...
        <!-- если заказ новый не обработан то выведу и передадим status 1 -->
            @if (!$order->status)
                <a href="{{route('blog.admin.order.change',$item->id)}}/?status=1" class="btn btn-success btn-xs">Одобрить</a>
                <a href="#" class="btn btn-warning btn-xs redact">Редактировать</a>
            @else
                <a href="{{route('blog.admin.order.change',$item->id)}}/?status=0" class="btn btn-default btn-xs">Вернуть на доработку</a>
            @endif

            <a class="btn btn-xs" href="">
                <form id="delform" method="post" action="{{route('blog.admin.order.destroy', $item->id)}}"
                      style="float: none">
                    @method('DELETE')
                    @csrf
        ...
        @component('blog.admin.components.breadcrumb')
            @slot('title') Редактирование заказа № {{$item->id}} @endslot
            @slot('parent') Главная @endslot
            @slot('order') Список заказов @endslot
            @slot('active') Заказа № {{$order->id}} @endslot
        @endcomponent
    </section>
                ...
                <div class="box">
                    <div class="box-body">
                        <div class="table-responsive">
                            <form action="{{route('blog.admin.order.save', $item->id)}}" method="post">
